...
const Panier = () => {
  ...
  const totalpanier = panierState.panier.reduce((total, item) => total + item.product.price * item.qt, 0).toFixed(2);
      ...
      {panierState.panier.length > 0 && (
        <div className="card mt-3">
          <div className="card-body">
            ...
            <p className="card-text">Montant total: {totalpanier} EUR</p>
          </div>
        </div>
      )}
    </div>
